...
// The WordAlignmentFunc type is an adapter to allow the use of ordinary
// function as WordAlignment mutator.
type WordAlignmentFunc func(context.Context, *ent.WordAlignmentMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f WordAlignmentFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	mv, ok := m.(*ent.WordAlignmentMutation)
	if !ok {
		return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.WordAlignmentMutation", m)
	}
	return f(ctx, mv)
}
