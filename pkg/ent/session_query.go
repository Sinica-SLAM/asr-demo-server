...
func (sq *SessionQuery) QuerySegments() *SegmentQuery {
	query := &SegmentQuery{config: sq.config}
...
func (sq *SessionQuery) WithSegments(opts ...func(*SegmentQuery)) *SessionQuery {
	query := &SegmentQuery{config: sq.config}
...
func (sq *SessionQuery) sqlAll(ctx context.Context) ([]*Session, error) {
	...
	if query := sq.withSegments; query != nil {
		...
		query.Where(predicate.Segment(func(s *sql.Selector) {
			s.Where(sql.InValues(session.SegmentsColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.session_segments
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "session_segments" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "session_segments" returned %v for node %v`, *fk, n.ID)
			}
